#!/bin/bash
# OAI 5G RISC-V 移植版 - rfsim 一键运行脚本 (进迭时空 K3 板子专用)
# 本脚本针对真实 RISC-V 板子 (如进迭时空 SpacemiT K3)，不需要 QEMU
# 用法: sudo ./run_rfsim_k3.sh [full|gnb_only|ue_only|ping|status|stop]
set -e
...
if [ ! -f "$BUILD_DIR/nr-softmodem" ] || [ ! -f "$BUILD_DIR/nr-uesoftmodem" ]; then
    echo "错误：二进制不存在，请先编译 (make nr-softmodem nr-uesoftmodem)"
    exit 1
fi

# K3 板子使用系统库（不需要 riscv-env/lib，板子系统已有）
export LD_LIBRARY_PATH="/usr/lib:/usr/lib/riscv64-linux-gnu"
# 注意：K3 板子上直接运行，不需要 qemu-riscv64
...
case "${1:-full}" in
    ...
    full)
        echo "============================================"
        echo "  OAI 5G RISC-V rfsim 完整运行 (K3 板子)"
        echo "============================================"
