 ...
 * Replaces s1ap_eNB_trace.h's .c: the mouse07410 asn1c output emits ANY_t for
...
int s1ap_eNB_handle_trace_start(uint32_t         assoc_id, uint32_t         stream, S1AP_S1AP_PDU_t *pdu) { return 0; }

int s1ap_eNB_handle_deactivate_trace(uint32_t         assoc_id, uint32_t         stream, S1AP_S1AP_PDU_t *pdu) { return 0; }
